...
export const miningService = {
  ...
  async deleteEquipment(id) {
    try {
      // Supprimer d'abord les enregistrements dépendants pour éviter les violations de contrainte
      await supabase.from('fuel_transactions').delete().eq('equipment_id', id);
      await supabase.from('equipment_operation_logs').delete().eq('equipment_id', id);
      await supabase.from('maintenance').delete().eq('equipment_id', id);
      await supabase.from('stock_entries').delete().eq('equipment_id', id);
      ...
      console.warn('Delete equipment failed, falling back to retire equipment', deleteError);
  ...
  async getStockSummary() {
    ...
    const DIMENSIONS = [
      '0/31,5', '15/25', '5/15', '10/14', '6/10', '4/6', '0/6', '0/5', '0/4', 'Minerai', 'Forage'
    ];
  ...
  async getDashboardStats() {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      weekDayMap[key] = { jour: DAY_LABELS[d.getDay()], production: 0, objectif: 0 };
    }
    ...
    productionsMonth.forEach(p => {
      ...
      const key = `S${weekNum}`;
      if (!weeklyMap[key]) weeklyMap[key] = { jour: key, production: 0, objectif: 0 };
  ...
  async updateOilTransaction(id, updates) {
    const { data, error } = await supabase
      .from('oil_transactions')
      .update(updates)
      .eq('id', id)
  ...
  // ============================================================
  // CARBURANT - suppression / mise à jour
  // ============================================================
  ...
  async updateFuelTransaction(id, updates) {
    const { data, error } = await supabase
      .from('fuel_transactions')
      .update(updates)
      .eq('id', id)
